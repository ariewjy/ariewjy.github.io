...
### Introduction
This project is made as a mandatory capstone for Data Science bootcamp. The project uses a new dataset from web-scraping an online ads listing for property to rent in Kuala Lumpur, Malaysia.[mudah.my](https://www.mudah.my/). The project explains the process of web-scraping, implementing a library such as beautifulsoup.

This project serves two important objectives:
1. To give a sensible pricing range, given a location, number of bathrooms and bedrooms, area, parking, etc.
1. The predicted price range may help potential tenant in getting a best price, similarly ads lister/agent/owner to get the best price for their units.

Some related works with similar project are listed below:

- https://sci-hub.ru/10.1109/ICSSS.2019.8882834
- https://arxiv.org/pdf/2210.06261.pdf
- https://arxiv.org/pdf/2209.05471.pdf
- https://arxiv.org/pdf/2301.01222.pdf
- https://towardsdatascience.com/predicting-airbnb-prices-with-machine-learning-and-deep-learning-f46d44afb8a6

Some are familiar with the boston house pricing dataset, which was not listed above. However, some papers listed are mentioning the use of Gradient boosting model or similar to that, to be the most reliable model. Other paper discussed about feature selections in predicting the house pricing. 

Author believes, even though the above work are all about house pricing, similar method can be applied to property rent pricing. 
...
#### Web-Scraping
```{python}
#getting listing property in one page
...
+ As seen above, nearby KTM/LRT is sligthly increases the median monthly rent by 50RM, **However**

```{python}
df[df['prop_name'] == 'Majestic Maxim'][['nearby_railways']].value_counts()
```

+ As can be seen from one example above, near KTM/LRT is not appearing in all row even though the property is the same
...
To remove some unexplainable data such as 0 monthly rent, 0 size, the rent that is way too old (1970), including the monthly rent that is way too high (astronomical), and size too big.

```{python}
#EDA to get the outliers 
...
One must paying attention to the number of categorical observation in the original data, with respect to the sampling train-test value.
If, the test_size = 0.3, that means any categorical observation with a total of 3 and less, would not be distributed evenly among train and test data. 
...
1. There are some factors that author believed to be affecting the result/ performance of the model:
    ...
    1. Feature selection importance can be seen on the last table, but initially the selection was based on paper and intuition of the author (author lives and work in KL, Malaysia for 5 years). Feature such as `completion_year` and `nearby_railways` are important in improving the model.
    1. Last but not least is the outlier identification. The best practice for me is using jointplot to see not only the distribution of the data in 2-dimension, but also in the third dimension (the density) of the data.

1. Some of the feature that were believed to be quite important even before doing the modeling is size, furnished and location. All three is available within the 10-most features affecting the modeling. As a context, location in KLCC is like Pondok Indah in South Jakarta. Location in Kiara is like BSD in South Tangerang.
