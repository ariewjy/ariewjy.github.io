...
I would like to make graphic where I can **colored a country based on its continent**, just for an additional context to the data. We took that from Gapminder dataset[^1], and combine it with the previously curated CO~2~ dataset we just created.
...
I would like to also see depending on the GDP per Capita, from which income class is a certain country belongs to. I am using a rough [classification by world-bank](https://blogs.worldbank.org/opendata/new-world-bank-country-classifications-income-level-2022-2023). I know this may not be the best representation as income class is not the same every year[^2], and I am not exactly using exact number here, but I think this is good enough to put some contexts for data illustration.
...
To prove a point that was alluded before, that __only rich countries was able to decoupled__ - let's see if there is a country, with large population (above 100mil), that are not categorized as rich country (above 4500 GDP per capita), that was able to Decoupled?
...
Only three countries, in the entire world - that was able to decoupled, when the population is at least 100 million. 
Furthermore, __there is No Country that was below the upper-middle income class was able to decoupled__. 

### Country with Decreased CO~2~ Emission & Decreased GDP

This is part where we should expect some of the poorest country in the world, where their CO~2~ emission should not be halted because it is directly related to the their economic growth. @fig-co2down-gdpdown shows country with worse condition in 2018 than they are in 1990, Zimbabwe. This country needs help, and if that means emitting more CO~2~ emission, so be it. In fact their carbon budget is still below their fair share, as shown in @fig-cumco2emission-uk-zimbabwe, a night and day difference compared to UK carbon share[^3].
